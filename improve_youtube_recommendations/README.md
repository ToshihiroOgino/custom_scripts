...
| Version | 変更内容                                                                                                                                                                                            |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.2.0   | オプションを `VIEW_COUNT_THRESHOLD` / `VERBOSE` / `DEBUG` の 3 つに整理。`REMOVAL_MODE`（`hide` で隠すだけにするモード）と `REMOVE_UNKNOWN_VIEW_COUNT` を廃止し、削除は常に DOM から取り除く／再生回数が読めないカードは常に残す挙動に固定。VERBOSE パネルは常に折りたたんだ状態で表示。 |
